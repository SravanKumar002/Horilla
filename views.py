...
#     if not joining_date:
#         raw_join = getattr(employee, "created_at", date.today())

#         # If created_at is datetime → convert to date
#         if isinstance(raw_join, datetime):
#             joining_date = raw_join.date()
#         else:
#             joining_date = raw_join
...
#         Q(start_date__lte=max(month_dates)) &
#         Q(end_date__gte=min(month_dates))
#     )

#     holiday_dates_set = set()
#     for hol in all_holidays:
#         start = max(hol.start_date, month_dates[0])
#         end = min(hol.end_date or hol.start_date, month_dates[-1])
#         cur = start
#         while cur <= end:
#             holiday_dates_set.add(cur)
...
#         employee_id=employee
#     )
#     wr_dict = {(wr.employee_id_id, wr.date): wr for wr in work_records}
...
#             # Fetch schedule safely if shift exists
#             if emp_shift:
...
#             if emp_shift and schedule:
#                 is_working_day = not getattr(schedule, "is_day_off", False)
#             elif emp_shift:
#                 is_working_day = True
#             else:
#                 is_working_day = True
...
#             # ======================================================================
#             #                CASE A: WORK RECORD EXISTS
#             # ======================================================================
#             if wr:
#                 wr_type = wr.work_record_type

#                 # Future
#                 if date_obj > today:
#                     future_map = {
#                         "WOF": "weekly-off" if not is_working_day else "missed-punch-in",
#                         "LFD": "leave-full",
#                         "LHD": "leave-half"
#                     }
#                     day_status = future_map.get(wr_type, "future")
...
#                                 worked_sec = attendance.at_work_second

#                         half_threshold = 0
#                         if emp_shift and schedule:
#                             minimum = strtime_seconds(schedule.minimum_working_hour)
#                             half_threshold = minimum // 2

#                         day_status = (
#                             "leave-half-present"
#                             if worked_sec >= half_threshold
#                             else "leave-half-absent"
#                         )
...
#                             "MCI": "missed-punch-in",
#                         }
#                         day_status = status_map.get(wr_type, "absent")

#             # ======================================================================
#             #                CASE B: NO WORK RECORD
#             # ======================================================================
#             else:
...
#                 else:
#                     # No activity?
#                     has_activity = AttendanceActivity.objects.filter(
#                         employee_id=employee,
#                         attendance_date=date_obj
#                     ).exists()

#                     if is_working_day and not has_activity:
#                         day_status = "missed-punch-in"
#                     else:
#                         day_status = "weekly-off" if not is_working_day else "absent"
    ...
    for week_index, week in enumerate(cal):
        for weekday_index, d in enumerate(week):
            ...
            if wr:
                ...
                if date_obj > today:
                    future_map = {
                        "WOF": "weekly-off" if not is_working_day else "absent",
                        "LFD": "leave-full",
                        "LHD": "leave-half"
                    }
                    day_status = future_map.get(wr_type, "future")
            ...
            # ======================================================================
                ...
                if date_obj > today:
                    ...
                    if is_working_day and not has_activity:
                        # ❌ removed missed-punch-in → now count as ABSENT
                        day_status = "absent"
                    else:
                        day_status = "weekly-off" if not is_working_day else "absent"
