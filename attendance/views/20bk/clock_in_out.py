...
def clock_in(request):
    ...
    if (
        ...
        if employee and work_info is not None:
            ...
            latest_checkin = (attendance_activities.filter(employee_id=employee, clock_in__isnull=False).order_by('-attendance_date', '-clock_in').first())
            ...
            if latest_checkin:
                ...
                print("elapsed_seconds in clockin view if", elapsed_seconds)
                
            else:
                print("elapsed_seconds in clockin view else", elapsed_seconds)
                elapsed_seconds = 0
            script = ""
            hidden_label = ""
            ...
            if time_runner_enabled:
                ...
                <script>
                        ...
                        run = 1;
                        var run_interval = true;
                        console.log("run_interval clockinout",run_interval)
                        sessionStorage.setItem('runInterval', {elapsed_seconds})
                        at_work_seconds = {elapsed_seconds};
                        console.log("elapsed_seconds in clockin", at_work_seconds);
                        console.log("click 3", at_work_seconds)
                    </script>
                    """.format(
        ...
        if time_runner_enabled:
            script = """
                ...
                $(document).ready(function () {{
                    $('.at-work-seconds').html(secondsToDuration(0))
                }});
                run = 0;
                var run_interval = false;
                sessionStorage.setItem('runInterval', "false")
                console.log("interval_id",interval_id)
                clearInterval(interval_id);
                at_work_seconds = {at_work_seconds_forecasted};
                console.log("click 4", at_work_seconds)
                </script>
            """.format(
                at_work_seconds_forecasted=employee.get_forecasted_at_work()[
                    "forecasted_at_work_seconds"
