...
def _prefill_shift_defaults(attendance):
    """
    Build default values for attendance fields based on the employee's shift schedule.
    """
    defaults = {}
    if not attendance or not attendance.attendance_date:
        return defaults
    ...
    schedule = (
        EmployeeShiftSchedule.objects.filter(shift_id=shift, day__day=day_name)
        .select_related("day")
        .first()
    )
    if not schedule:
    ...
    attendance_date_str = attendance.attendance_date.strftime("%Y-%m-%d")

    if schedule.start_time and not attendance.attendance_clock_in:
        defaults.setdefault("attendance_clock_in", schedule.start_time.strftime("%H:%M"))
        defaults.setdefault("attendance_clock_in_date", attendance_date_str)

    if schedule.end_time and not attendance.attendance_clock_out:
        defaults.setdefault("attendance_clock_out", schedule.end_time.strftime("%H:%M"))
        defaults.setdefault("attendance_clock_out_date", attendance_date_str)

    if schedule.minimum_working_hour:
        defaults.setdefault("minimum_hour", schedule.minimum_working_hour)
        defaults.setdefault("attendance_worked_hour", schedule.minimum_working_hour)
...
def build_month_overview(request, attendances_queryset):
    """
    Build the contextual data required to display every day of the month for the selected employee.
    Returns dictionary containing day rows, late penalty maps and helper metadata.
    """
    ...
    schedule_map = {}
    if shift_obj:
        schedules = shift_obj.employeeshiftschedule_set.select_related("day").all()
        for schedule in schedules:
            day_name = getattr(getattr(schedule, "day", None), "day", "")
            if day_name:
                schedule_map[day_name.lower()] = schedule
    ...
    for entry in late_entries:
        ...
        late_map.setdefault(att.id, []).append(info)
        late_penalties_map[att.id] = late_penalties_map.get(att.id, 0) + info["penalties"]

    # Get leave dates for the employee
    leave_dates_set = set()
    if employee and apps.is_installed("leave"):
        ...
        approved_leaves = LeaveRequest.objects.filter(
            ...
            status="approved",
            start_date__lte=last_day_date,
            end_date__gte=first_day
        )
        for leave in approved_leaves:
            leave_dates = leave.requested_dates()
            for leave_date in leave_dates:
                if first_day <= leave_date <= last_day_date:
                    leave_dates_set.add(leave_date)

    # Get holiday dates
    from base.models import Holidays
    from django.db.models import Q
    all_holidays = Holidays.objects.filter(
        Q(start_date__lte=last_day_date) & 
        Q(end_date__gte=first_day)
    )
    holiday_dates_set = set()
    for hol in all_holidays:
        hol_start = max(hol.start_date, first_day)
        hol_end = min(hol.end_date or hol.start_date, last_day_date)
        day = hol_start
        while day <= hol_end:
    ...
    if employee:
        ...
        if work_info and getattr(work_info, "joining_date", None):
            joining_date = work_info.joining_date
        else:
    ...
    for day_number in range(1, last_day + 1):
        ...
        if joining_date and cur_date < joining_date:
            days.append(
                {
                    "date": cur_date,
                    ...
                    "penalties": 0,
                    "status": "not-joined",
                    "status_display": _("Not Joined"),
                    "row_class": "row-status--gray",
                    "request_status": "not-joined",
        ...
        schedule = schedule_map.get(weekday_key)
        is_weekoff = bool(shift_obj) and schedule is None
        is_public_holiday = cur_date in holiday_dates_set
        is_leave = cur_date in leave_dates_set

        min_halfday_hours = 4.0
        min_fullday_hours = 8.0
        if schedule and schedule.minimum_working_hour:
            min_hour_sec = strtime_seconds(schedule.minimum_working_hour)
            min_fullday_hours = min_hour_sec / 3600.0
            min_halfday_hours = min_fullday_hours / 2.0
        ...
        row_class = "row-status--red"

        if att:
            clock_in = att.attendance_clock_in
            clock_out = att.attendance_clock_out
            work_hours = duration_to_hours(att.attendance_worked_hour)
            if not work_hours and att.at_work_second:
                work_hours = att.at_work_second / 3600.0

            if att.is_validate_request_approved:
                validation_state = "approved"
                row_class = "row-status--blue"
            elif att.attendance_validated:
                validation_state = "validated"
                row_class = "row-status--green"
            elif att.is_validate_request:
                validation_state = "requested"
                row_class = "row-status--orange"

            if att.id in late_map:
        ...
        status_result = get_attendance_status(
            work_hours=work_hours,
            min_halfday_hours=min_halfday_hours,
            min_fullday_hours=min_fullday_hours,
            is_weekly_off=is_weekoff,
            is_leave=is_leave,
        ...
        logic_map = {
            "Public Holiday": ("public-holiday", _("Public Holiday")),
            "Leave": ("leave", _("Leave")),
            "Weekly Off": ("weekoff", _("Weekly Off")),
            "Present": ("present", _("Present")),
            "Half Day": ("half-day", _("Half Day")),
            "Absent (No Clock In/Out)": ("absent", _("Absent")),
            "Absent (Work hours < minimum half-day hours)": ("absent", _("Absent")),
            "Unknown": ("absent", _("Absent")),
        }

        status_key, status_display = logic_map.get(
            status_result, ("absent", _("Absent"))
        )

        if cur_date > today and status_key in ("present", "absent", "half-day"):
            status_key = "future"
            status_display = _("Future")
        ...
        days.append(
            {
                ...
                "row_class": row_class,
                "request_status": validation_state,
                "shift_label": str(att.shift_id) if att and att.shift_id else shift_label,
                "work_type_label": str(att.work_type_id)
                if att and att.work_type_id
                else work_type_label,
...
def calendar_work_records_change_month(request):
    ...
    for day in all_days:
        attendance = Attendance.objects.filter(attendance_date=day).first()
        activities = AttendanceActivity.objects.filter(attendance_date=day)
    ...
    if is_bulk:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                messages.success(
                    request,
                    _("Attendance request created for {} employee(s)").format(
...
def attendance_request_changes(request, attendance_id):
    ...
    shift_defaults = _prefill_shift_defaults(attendance)
    if request.GET.get("previous_url"):
        form = AttendanceRequestForm(initial=request.GET.dict())
    else:
        form = AttendanceRequestForm(instance=attendance, initial=shift_defaults)
