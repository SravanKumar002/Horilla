...
def contract_info_initial(request):
    """
    AJAX view to return employee contract-related info.
    Used to auto-fill contract form fields when an employee is selected.
    """
    ...
    def safe_value(value, default=0):
        """Ensure all numeric and date values are JSON serializable."""
        if value is None:
            return default
    ...
    try:
        # 1️⃣ Try to fetch ACTIVE contract for the employee
        contract = (
            Contract.objects.filter(employee_id=employee_id, contract_status="active")
            .select_related(
        ...
        if contract:
            return JsonResponse({
                "department": contract.department.id if contract.department else "",
                "job_position": contract.job_position.id if contract.job_position else "",
                "job_role": contract.job_role.id if contract.job_role else "",
                "shift": contract.shift.id if contract.shift else "",
                "work_type": contract.work_type.id if contract.work_type else "",
                "wage": safe_value(contract.wage),
                "contract_start_date": safe_value(contract.contract_start_date, ""),
                "contract_end_date": safe_value(contract.contract_end_date, ""),
                "house_allowance": safe_value(contract.house_allowance),
                "transport_allowance": safe_value(contract.transport_allowance),
                "other_allowance": safe_value(contract.other_allowance),
                "total_salary": safe_value(contract.total_salary),
                "source": "contract",
            })

        # 2️⃣ No active contract found — use EmployeeWorkInformation
        work_info = (
            EmployeeWorkInformation.objects.filter(employee_id=employee_id)
            .select_related(
                "department_id", "job_position_id", "job_role_id",
                "shift_id", "work_type_id"
            )
            .first()
        )
        ...
        if work_info:
            total_salary = (
                safe_value(work_info.basic_salary, 0)
                + safe_value(work_info.house_allowance, 0)
                + safe_value(work_info.transport_allowance, 0)
                + safe_value(work_info.other_allowance, 0)
            )
            ...
            return JsonResponse({
                ...
                "wage": safe_value(work_info.basic_salary),
                "contract_start_date": safe_value(work_info.date_joining, ""),
                "contract_end_date": safe_value(work_info.contract_end_date, ""),
                "house_allowance": safe_value(work_info.house_allowance),
                "transport_allowance": safe_value(work_info.transport_allowance),
                "other_allowance": safe_value(work_info.other_allowance),
                "total_salary": total_salary,
                "source": "work_info",
            })

        # 3️⃣ No data at all — new employee
        return JsonResponse({
            "department": "",
            "job_position": "",
            "job_role": "",
