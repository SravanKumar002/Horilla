...
def payroll_calculation(employee, start_date, end_date):
    ...
    print("Dynamic Allowances:", allowances["allowances"])
    ...
    house_allowance = updated_gross_pay_data.get("house_allowance", 0.0)
    ...
    total_allowance += house_allowance + transport_allowance + other_allowance
    kwargs["gross_pay"] = gross_pay

    print("✅ Gross Pay:", gross_pay)
    print("House:", house_allowance, "| Transport:", transport_allowance, "| Other:", other_allowance)
...
def payslip_export(request):
    ...
    for ps in payslips:
        ...
        house_allowance = round(float(data.get("house_allowance", 0)), 2)
        ...
        def extract_allowances(obj):
            nonlocal house_allowance, transport_allowance, other_allowance
            ...
            if isinstance(obj, list):
                ...
                if amount:
                    if any(x in name for x in ["house", "house", "hra", "rent"]):
                        house_allowance += amount
        ...
        if total_allowance == 0:
            ...
            print(
                f"✅ Allowances for {emp_code}: House={house_allowance}, Transport={transport_allowance}, Other={other_allowance}, Total={total_allowance}"
            )
        ...
        lop = round(float(data.get("loss_of_pay", 0)), 2)
        
        # Total Deductions is the sum of all list deductions (other) and LOP
        total_deductions = round(other_deductions_amount + lop, 2)
        ...
        export_rows.append({
            ...
            "End Date": end_date,
            "Total Paid days": paid_days, # <-- NEW COLUMN
            "Basic Pay": basic_pay,
            "Gross Pay": gross_pay,
            "Net Pay": net_pay,
            "House Allowance": round(house_allowance, 2),
            "Transport Allowance": round(transport_allowance, 2),
            "Other Allowance": round(other_allowance, 2),
            "Total Allowances": round(total_allowance, 2),
            "LOP": lop, # <-- NEW COLUMN
            "Other Deductions": other_deductions_amount, # <-- NEW COLUMN
    ...
    # --- Excel Formatting Code (FIXED bg_color #) ---
    header_format = workbook.add_format({
        ...
        "align": "center",
        "font_color": "#FFFFFF",
        "bg_color": "#7A34AB", # <-- Fixed the double #, using a dark purple color
        "border": 1,
    })

    # Format Excel headers and columns
