...
def view_created_payslip(request, payslip_id, **kwargs):
    """
    View a saved payslip and calculate total salary including allowances
    and subtract total_deductions from the net salary.
    """
    payslip = Payslip.objects.filter(id=payslip_id).first()
    ...
    if payslip is not None and (
        ...
        # 💡 Step 1: Get EmployeeWorkInformation
        ...
        # 💡 Step 2: Get salary & allowances from EmployeeWorkInformation
        basic_salary = float(employee_work_info.basic_salary or 0) if employee_work_info else 0.0
        house_allowance = float(employee_work_info.house_allowance or 0) if employee_work_info else 0.0
        transport_allowance = float(employee_work_info.transport_allowance or 0) if employee_work_info else 0.0
        other_allowance = float(employee_work_info.other_allowance or 0) if employee_work_info else 0.0

        # 💡 Step 3: Total from fixed fields
        total_salary = basic_salary + house_allowance + transport_allowance + other_allowance

        # 💡 Step 4: Add dynamic allowances from payslip data
        extra_allowances_total = sum(
            float(a.get("amount", 0.0)) for a in data.get("allowances", [])
        )
        total_salary += extra_allowances_total

        # 💡 Step 5: Subtract total_deductions key
        deductions_total = float(data.get("total_deductions", 0.0))

        # 💡 Step 6: Calculate final net salary
        net_salary = total_salary - deductions_total

        # 💡 Step 7: Update payslip fields (optional but recommended)
        payslip.basic_pay = basic_salary
        payslip.house_allowance = house_allowance
        payslip.transport_allowance = transport_allowance
        payslip.other_allowance = other_allowance
        payslip.gross_pay = total_salary
        payslip.deduction = deductions_total
        payslip.net_pay = net_salary
        payslip.save()

        # 💡 Step 8: Add everything to context for template
        data["employee"] = payslip.employee_id
        data["payslip"] = payslip
        data["basic_salary"] = basic_salary
        data["house_allowance"] = house_allowance
        data["transport_allowance"] = transport_allowance
        data["other_allowance"] = other_allowance
        data["extra_allowances_total"] = extra_allowances_total
        data["deductions_total"] = deductions_total
        data["total_salary"] = total_salary  # before deductions
        data["net_salary"] = net_salary      # after deductions

        # 💡 Step 9: JSON safe copy
        data["json_data"] = data.copy()
        data["json_data"]["employee"] = payslip.employee_id.id
        data["json_data"]["payslip"] = payslip.id
        data["instance"] = payslip
        ...
        print(f"Employee: {payslip.employee_id}")
        print(f"Basic Salary: {basic_salary}")
        print(f"Allowances (Work Info): {house_allowance}, {transport_allowance}, {other_allowance}")
        print(f"Extra Allowances Total: {extra_allowances_total}")
        print(f"Total Deductions (from key): {deductions_total}")
