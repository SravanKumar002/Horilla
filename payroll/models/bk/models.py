...
from django.apps import apps
from datetime import date # Added import for date.today()

# Assuming HorillaModel, Employee, FilingStatus, Department, JobPosition, 
# JobRole, EmployeeShift, WorkType, HorillaAuditLog, HorillaAuditInfo, 
# HorillaCompanyManager, min_zero, and upload_path are imported elsewhere.
...
class Contract(HorillaModel):
    ...
    try:
        # Note: This is usually done outside the class definition or handled via a manager
    ...
    # --- Compensation Fields (Corrected, single definition) ---
    wage = models.FloatField(verbose_name=_("Basic Salary"), null=True, default=0)
    house_allowance = models.FloatField(
        default=0, null=True, blank=True, verbose_name=_("House Allowance")
    )
    transport_allowance = models.FloatField(
        default=0, null=True, blank=True, verbose_name=_("Transport Allowance")
    )
    other_allowance = models.FloatField(
        default=0, null=True, blank=True, verbose_name=_("Other Allowance")
    )
    total_salary = models.FloatField( # Corrected case from 'Total Salary'
        default=0, null=True, blank=True, verbose_name=_("Total Salary")
    )
    # --- End Compensation Fields ---
    ...
    def clean(self):
        ...
        if self.contract_status == "active" and Contract.objects.filter(
            employee_id=self.employee_id, contract_status="active"
        ).exclude(id=self.pk).exists():
            raise forms.ValidationError(_("An active contract already exists for this employee."))

        if self.contract_status == "draft" and Contract.objects.filter(
            employee_id=self.employee_id, contract_status="draft"
        ).exclude(id=self.pk).exists():
            raise forms.ValidationError(_("A draft contract already exists for this employee."))

        if self.wage_type in ["daily", "monthly"] and not self.calculate_daily_leave_amount:
    ...
    def save(self, *args, **kwargs):
        # Fill missing info from EmployeeWorkInformation
        if EmployeeWorkInformation.objects.filter(employee_id=self.employee_id).exists():
            # ... (Logic to fill fields from employee_work_info remains the same) ...
        ...
        if self.contract_end_date and self.contract_end_date < date.today():
            self.contract_status = "expired"

        # Calculate total_salary before saving
        self.total_salary = (
            (self.wage or 0)
