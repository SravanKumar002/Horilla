...
def calculate_gross_pay(*_args, **kwargs):
    """
    Calculates the Gross Pay by summing Basic Pay, Dynamic Allowances, and Fixed Contract Allowances (pro-rata).
    Updates kwargs with fixed allowance details for payslip data and RETURNS the final gross_pay value.
    """
    employee = kwargs["employee"]
    # basic_pay को Decimal में लें ताकि floating point issues कम हों
    basic_pay = Decimal(kwargs["basic_pay"]) 
    # total_allowance yaha par dynamic allowances ka total hona chahiye.
    total_dynamic_allowance = Decimal(kwargs.get("total_allowance", 0.0)) 
    start_date = kwargs.get("start_date")
    end_date = kwargs.get("end_date")

    # --- Fixed Allowances Contract से लें (start_date, end_date पास करें) ---
    # get_contract_fixed_allowances को भी start_date/end_date की ज़रूरत है pro-rata के लिए
    (
        house_a,
        transport_a,
        other_a,
    ...
    # Total Allowance को अपडेट करें (dynamic + fixed)
    # total_allowance को Decimal में रखें
    total_allowance = total_dynamic_allowance + Decimal(total_fixed_allowances)
    
    # Gross Pay की गणना (basic_pay + total_allowance)
    gross_pay = basic_pay + total_allowance

    # --- Fixed Allowances को payslip data (kwargs) में store करें ---
    # payslip_data में top-level keys को update करना (ये key log में दिख रहे थे)
    # इन्हें float में बदलकर kwargs में स्टोर करें
    kwargs["house_allowance"] = float(house_a)
    kwargs["transport_allowance"] = float(transport_a)
    kwargs["other_allowance"] = float(other_a)
    
    # Baki calculation steps aur database saving ke liye kwargs mein update karna zaroori hai.
    kwargs["total_allowance"] = float(total_allowance)
    kwargs["gross_pay"] = float(gross_pay) # gross_pay को kwargs में float में अपडेट करें
    
    # --- IMPORTANT: Return the calculated gross_pay value ---
    # यह सुनिश्चित करता है कि अगला स्टेप (जैसे taxable_gross_pay) सही मान का उपयोग करे।
    return float(gross_pay)
