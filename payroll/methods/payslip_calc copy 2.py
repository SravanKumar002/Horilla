...
try:
    ...
    # Fallback/Debug: Log if the required utility functions are missing
    print("WARNING: Could not import get_total_days_in_month or get_total_days from payroll.methods.methods. Pro-Rata calculation may fail.")
    def get_total_days_in_month(d): return 30 # Placeholder fallback
    def get_total_days(s, e): return 30 # Placeholder fallback
...
def calculate_based_on_gross_pay(*_args, **kwargs):
    ...
    # --- Payslip Data (kwargs) में Allowances और Gross Pay को स्टोर करें ---
    # CRITICAL FIX: house_allowance, transport_allowance, other_allowance सीधे kwargs में स्टोर करें
    # ताकि methods.py का save_payslip इसे Payslip Model में save कर सके।
    kwargs["house_allowance"] = float(house_a)
    kwargs["transport_allowance"] = float(transport_a)
    kwargs["other_allowance"] = float(other_a)
    
    # total_allowance (dynamic + fixed) और gross_pay को अपडेट करें
    kwargs["total_allowance"] = float(total_allowance)
    kwargs["gross_pay"] = float(gross_pay) 
    
    # 'pay_head_data' JSON field mein fixed allowances ko store karna 
    if "pay_head_data" not in kwargs:
        kwargs["pay_head_data"] = {}
    
    # Fixed contract allowances details ko pay_head_data mein dalen
    kwargs["pay_head_data"]["contract_allowances"] = {
        "house_allowance": float(house_a),
        "transport_allowance": float(transport_a),
        "other_allowance": float(other_a),
        "total_fixed_allowances": float(total_fixed_allowances)
    }
    
    # --- CRITICAL FIX: Return the calculated gross_pay value ---
    # यह ensures करता है कि अगला स्टेप (जैसे taxable_gross_pay) सही मान का उपयोग करे।
    return float(gross_pay)

def calculate_based_on_net_pay(*_args, **kwargs):
    # Placeholder
    # Net Pay calculation logic yahan aayega (Taxable Gross Pay - Tax - Deductions)
...
def get_fixed_contract_allowances(contract, paid_days_in_period, total_days_in_month):
    ...
    # Apply pro-rata calculation to fixed allowance fields
    allowances["house_allowance"] = contract.house_allowance * pro_rata_factor
...
def calculate_gross_pay(*_args, **kwargs):
    ...
    """
    Properly calculate and return Gross Pay including fixed contract allowances.
    """
    employee = kwargs["employee"]
    basic_pay = Decimal(kwargs.get("basic_pay", 0.0))
    total_dynamic_allowance = Decimal(kwargs.get("total_allowance", 0.0))
    start_date = kwargs.get("start_date")
    end_date = kwargs.get("end_date")
    ...
    if not contract:
        # No contract found
        kwargs["house_allowance"] = 0.0
        kwargs["transport_allowance"] = 0.0
        kwargs["other_allowance"] = 0.0
        kwargs["gross_pay"] = float(basic_pay + total_dynamic_allowance)
        return {
            "gross_pay": kwargs["gross_pay"],
            "house_allowance": 0.0,
            "transport_allowance": 0.0,
            "other_allowance": 0.0,
            "deductions": [],
    ...
    # ✅ Pro-rata calculation for fixed allowances
    pro_rata_factor = paid_days / total_days_in_month if total_days_in_month else 0
    house_allowance = float(contract.house_allowance * pro_rata_factor)
    transport_allowance = float(contract.transport_allowance * pro_rata_factor)
    other_allowance = float(contract.other_allowance * pro_rata_factor)
    total_fixed_allowance = house_allowance + transport_allowance + other_allowance

    total_allowance = float(total_dynamic_allowance + Decimal(total_fixed_allowance))
    gross_pay = float(basic_pay + Decimal(total_allowance))

    # ✅ Store values in kwargs
    kwargs.update({
...
def get_contract_fixed_allowances(employee_id, start_date, end_date):
    """
    Fetches house_allowance, transport_allowance, and other_allowance
    from the employee's active contract, calculated on a pro-rata basis.
    """
    contract = Contract.objects.filter(employee_id=employee_id, is_active=True).first()
    print("DEBUG CONTRACT for fixed allowances:", contract)
    house_a = Decimal(0.0)
    transport_a = Decimal(0.0)
    other_a = Decimal(0.0)
    total_fixed_allowances = Decimal(0.0)
    pro_rata_factor = Decimal(0)

    if contract and start_date and end_date:
        try:
            # Date objects में conversion सुनिश्चित करें
            ...
            # Period के कुल दिन (जैसे 24 दिन)
            paid_days = get_total_days(start_date, end_date) 
            # Period के start_date वाले महीने के कुल दिन (जैसे अक्टूबर के 31 दिन)
            total_days_in_month = get_total_days_in_month(start_date)
             
            if total_days_in_month > 0:
                 pro_rata_factor = Decimal(paid_days) / Decimal(total_days_in_month)
            
            # Allowances को Decimal में लेकर calculation करें
            monthly_house_a = Decimal(contract.house_allowance or 0)
            monthly_transport_a = Decimal(contract.transport_allowance or 0)
            monthly_other_a = Decimal(contract.other_allowance or 0)

            house_a = monthly_house_a * pro_rata_factor
            transport_a = monthly_transport_a * pro_rata_factor
            other_a = monthly_other_a * pro_rata_factor

        except Exception as e:
            # Log calculation errors
            print(f"Error in fixed allowance pro-rata calculation: {e}")


        total_fixed_allowances = house_a + transport_a + other_a
        
    # Return as float as expected by the calling function
    return float(house_a), float(transport_a), float(other_a), float(total_fixed_allowances)
